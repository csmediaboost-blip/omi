...
export async function activateGPUNode(
  ...
  // SECURITY 2.5: Prevent GPU node double activation
  // Check if user already has an active allocation for this node type
  const now_check = new Date();
  const { data: existingActive } = await supabaseAdmin
    .from("node_allocations")
    .select("id, created_at, maturity_date, status")
  ...
  if (existingActive && existingActive.length > 0) {
    ...
    // Check if previous activation hasn't expired
    const maturityDate = active.maturity_date ? new Date(active.maturity_date) : null;
    const isExpired = maturityDate && maturityDate < now_check;
    
    if (!isExpired) {
      console.warn(`[FRAUD] User ${payment.user_id} attempted to double-activate GPU node ${payment.node_key}`);
      throw new Error("GPU node already activated for this user. Previous activation is still active.");
    }
  }

  // Idempotency check — avoid duplicate allocations from same payment
