...
import {
  Receipt, DollarSign, Shield, AlertTriangle, CheckCircle,
  XCircle, Clock, RefreshCw, TrendingDown, TrendingUp, Server,
  ChevronDown, ChevronUp, ArrowUpRight, Wallet, BarChart3,
  CreditCard, Lock, Activity, Send, X, Pickaxe, Cpu,
} from "lucide-react";
...
type UserProfile = {
  ...
  balance_available: number; balance_pending: number; balance_locked: number;
  wallet_balance: number; pending_balance: number;
  total_earned: number; earnings: number;
  earning_withrawn: number; total_withrawn: number;
  weekly_withdrawn: number; last_withhrawal_at: string | null;
  kyc_verified: boolean; kyc_status: string;
  ...
  approved_count: number; rejected_countb: number;
  qaulity_score: number; streak_count: number;
  last_active_at: string | null; created_at: string;
};

// FIX #5: Extended with plan name from join
type NodeAllocation = {
  id: string; user_id: string; plan_id: string; amount_invested: number;
  currency: string; payment_model: string;
  contract_months: number | null; contract_label: string | null;
  contract_min_pct: number | null; contract_max_pct: number | null;
  maturity_date: string | null; lock_in_label: string; status: string;
  total_earned: number; total_withdrawn: number;
  mining_period: string | null; mining_ends_at: string | null;
  mining_completed: boolean; final_profit: number | null;
  created_at: string;
  // Joined from gpu_plans
  plan_name?: string; gpu_model?: string;
};

type PaymentTx = {
  id: number; user_id: string; node_key: string;
  amount: number; currency: string; gateway: string;
  gateway_reference: string | null; status: string;
  crypto_currency: string | null; crypto_amount: number | null;
  metadata: string | null; created_at: string;
  confirmed_at: string | null; verified_by_admin: boolean | null;
};

type OperatorLicense = {
  id: string; user_id: string; license_type: string;
  status: string; expires_at: string | null;
  purchased_at: string | null; payment_id: string | null;
};

type LedgerTx = {
  id: string; type: string; amount: number;
  balance_after: number | null; description: string;
  reference_id: string | null; metadata: Record<string, any>;
  created_at: string;
};

type Withdrawal = {
  id: string | number; amount: number; wallet_address: string;
  status: string; tracking_status?: string;
  expected_date?: string | null; created_at: string;
  paid_at: string | null; failure_reason: string | null;
  payout_account_name?: string | null; payout_method?: string | null;
};
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border capitalize ${map[status] ?? "bg-slate-800 border-slate-700 text-slate-400"}`}>
      {status}
    </span>
  );
}

function StatBox({ label, value, color, icon: Icon, sub }: {
  label: string; value: string | number; color: string;
  icon: React.ComponentType<{ size?: number; className?: string }>; sub?: string;
}) {
  return (
    <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-4">
      <div className={`w-8 h-8 bg-slate-800 rounded-xl flex items-center justify-center mb-2 ${color}`}>
        <Icon size={15} />
      </div>
...
function GatewayBadge({ gateway }: { gateway: string }) {
  if (gateway === "crypto_wallet")
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-violet-900/30 border border-violet-700/40 text-violet-400">₿ Crypto</span>;
  if (gateway === "bank_transfer")
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-900/30 border border-blue-700/40 text-blue-400">🏦 Bank Transfer</span>;
  if (gateway === "gpu_mining")
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-900/30 border border-emerald-700/40 text-emerald-400">⛏️ GPU Mining</span>;
  return <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-400">💳 Card</span>;
}
...
function WithdrawModal({
  userId, availableBalance, isFrozen, profile, onClose, onSuccess,
}: {
  userId: string; availableBalance: number; isFrozen: boolean;
  profile: UserProfile; onClose: () => void; onSuccess: () => void;
}) {
  ...
  const hasPayout     = !!(profile.payout_registered && profile.payout_account_number);
  const kycOk         = isKYCApproved(profile);

  async function handleSubmit() {
    setError("");
    if (!pin || pin.length < 4) { setError("Please enter your PIN (4–6 digits)"); return; }
    ...
    if (!ud?.pin_hash || pinHash !== ud.pin_hash) {
      ...
      logWithdrawalEvent(supabase, userId, "withdrawal_failed", { reason: "Invalid PIN", amount: amt }).catch(() => {});
      return;
    }

    // Security checks
    const check = await runWithdrawalSecurityChecks(supabase, userId, amt, profile as UserSecurityProfile);
    if (!check.pass) {
      setError(check.reason);
      logWithdrawalEvent(supabase, userId, "withdrawal_failed", { reason: check.reason, amount: amt }).catch(() => {});
      return;
    }
    ...
    try {
      ...
      if (!deduct.success) {
        ...
        logWithdrawalEvent(supabase, userId, "withdrawal_failed", { reason: "Balance deduction failed", amount: amt }).catch(() => {});
        setLoading(false);
        return;
      }
      ...
      if (wErr) {
        ...
        logWithdrawalEvent(supabase, userId, "withdrawal_failed", { reason: wErr.message, amount: amt }).catch(() => {});
        throw wErr;
      }

      await recordWithdrawalLedger(supabase, userId, amt, payoutAcct, payoutGateway);
      logWithdrawalEvent(supabase, userId, "withdrawal_requested", {
        amount: amt, payout_method: payoutGateway,
      ...
      logWithdrawalEvent(supabase, userId, "withdrawal_failed", { reason: e.message ?? "Unknown", amount: amt }).catch(() => {});
    }
    setLoading(false);
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl overflow-hidden flex flex-col"
        style={{ background: "rgb(10,16,28)", border: "1px solid rgba(16,185,129,0.3)", maxHeight: "90vh" }}
        ...
        <div className="px-5 py-4 flex items-center justify-between flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs">Admin processes to your registered account</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          {!kycOk && (
            <div className="rounded-xl p-3" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)" }}>
              <p className="text-red-400 text-sm font-bold flex items-center gap-2"><AlertTriangle size={13} /> KYC verification required</p>
              <p className="text-red-400/70 text-xs mt-1">Complete identity verification in the Verification section. Mining continues unaffected.</p>
            </div>
          )}
          ...
          <div className="rounded-xl p-4" style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)" }}>
            <p className="text-slate-400 text-[10px] uppercase tracking-wide mb-2 flex items-center gap-1.5">
              <Shield size={9} className="text-blue-400" /> Registered Payout Account
            </p>
            {hasPayout ? (
              <div className="space-y-0.5">
                <p className="text-white font-bold text-sm">{payoutName}</p>
                {payoutBank && <p className="text-slate-400 text-xs">{payoutBank}</p>}
                <p className="text-slate-500 text-xs font-mono">{payoutAcct}</p>
                <p className="text-blue-400 text-[10px] capitalize">via {payoutGateway}</p>
                {kycOk && <p className="text-emerald-400 text-[10px] flex items-center gap-1"><CheckCircle size={9} /> KYC verified</p>}
              </div>
            ) : (
              <div>
                <p className="text-red-400 text-sm font-bold">No payout account registered</p>
          ...
          {!hasPayout && (
            <div className="rounded-xl p-3" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)" }}>
              <p className="text-red-400 text-sm font-bold flex items-center gap-2">
                <Lock size={13} /> Cannot withdraw without a registered payout account
              </p>
            </div>
          )}

          <div className="rounded-xl p-4" style={{ background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.2)" }}>
            <p className="text-slate-500 text-[10px] uppercase tracking-wide mb-1">Available Balance</p>
            <p className="text-emerald-400 font-black text-2xl">${available.toFixed(4)}</p>
            <p className="text-slate-600 text-xs mt-0.5">Minimum withdrawal: $10.00</p>
          </div>

          <div>
            <label className="text-slate-300 text-sm font-bold block mb-2">Amount (USD)</label>
            <div className="relative">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-lg">$</span>
              <input type="number" min={MIN} max={available} value={amount}
            ...
            <div className="flex gap-2 mt-2">
              {[25, 50, 75, 100].map((p) => (
                <button key={p} onClick={() => setAmount(((available * p) / 100).toFixed(2))}
          ...
          <div className="rounded-xl p-3"
            style={{ background: isBizDay ? "rgba(16,185,129,0.08)" : "rgba(239,68,68,0.08)", border: isBizDay ? "1px solid rgba(16,185,129,0.2)" : "1px solid rgba(239,68,68,0.25)" }}>
            <p className={`text-sm font-bold flex items-center gap-2 ${isBizDay ? "text-emerald-400" : "text-amber-400"}`}>
              <Clock size={13} />{bizMsg}
            </p>
            {!isBizDay && <p className="text-amber-400/70 text-xs mt-1">Withdrawals only processed on business days (Mon–Fri).</p>}
          </div>

          <div>
            <label className="text-slate-300 text-sm font-bold block mb-2">Security PIN <span className="text-red-400">*</span></label>
          ...
          {amt >= MIN && amt <= available && (
            <div className="rounded-xl p-4 space-y-2" style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)" }}>
              <p className="text-blue-300 text-xs font-black uppercase tracking-wide">Settlement Timeline</p>
              {[
                { label: "Queued",     desc: "Request received",            done: true,  active: false },
                { label: "Processing", desc: "Admin verifying",             done: false, active: true  },
                { label: "In Transit", desc: amt < 500 ? "Same day" : "Batch", done: false, active: false },
                { label: "Paid",       desc: `Expected ${expDate.toLocaleDateString()}`, done: false, active: false },
              ].map((s) => (
                <div key={s.label} className="flex items-start gap-3">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 ${s.done ? "bg-emerald-500 border-emerald-500" : s.active ? "border-blue-400 animate-pulse" : "border-slate-700"}`}>
                    {s.done && <CheckCircle size={9} className="text-white" />}
                  </div>
                  <div>
                    <p className={`text-xs font-bold ${s.done ? "text-emerald-400" : s.active ? "text-blue-300" : "text-slate-600"}`}>{s.label}</p>
          ...
          <button onClick={handleSubmit}
            disabled={loading || isFrozen || !amount || !hasPayout || !kycOk || pin.length < 4 || !isBizDay}
            ...
            {loading         ? <><RefreshCw size={15} className="animate-spin" /> Submitting…</>
             : !isBizDay     ? <><Clock size={15} /> Available Mon–Fri only</>
             : pin.length < 4 ? <><Lock size={15} /> Enter PIN to continue</>
...
export default function FinancialsPage() {
  ...
  const [gpuPlans,       setGpuPlans]       = useState<Record<string, { name: string; gpu_model: string }>>({});
  const [loading,        setLoading]        = useState(true);
  const [tab,            setTab]            = useState<"overview"|"deposits"|"investments"|"licenses"|"ledger"|"withdrawals">("overview");
  ...
  const loadData = useCallback(async () => {
    ...
    const [profRes, payRes, nodeRes, licRes, ledgerRes, wRes, plansRes] =
      await Promise.allSettled([
        ...
        supabase.from("node_allocations").select(
        ...
        supabase.from("operator_licenses").select(
          "id, user_id, license_type, status, expires_at, purchased_at, payment_id"
        ).eq("user_id", user.id).order("purchased_at", { ascending: false }).limit(20),

        supabase.from("transaction_ledger").select(
    ...
    if (payRes.status === "fulfilled")
      setPaymentTxs(payRes.value.data ?? []);
    ...
    if (plansRes.status === "fulfilled" && plansRes.value.data) {
      ...
      for (const p of plansRes.value.data) map[p.id] = { name: p.name, gpu_model: p.gpu_model };
      setGpuPlans(map);
    }
  ...
  // ── REALTIME: Finance updates instantly when admin confirms payment ─────────
  // Listens for:
  //  • payment_transactions status change (pending → confirmed)
  //  • node_allocations INSERT (new mining session activated)
  //  • users balance_available change (mining credited)
  useEffect(() => {
    if (!userId) return;
    ...
    const financeChannel = supabase
      ...
      .on(
        ...
        () => { loadData(); }, // Reload when payment status changes
      )
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "payment_transactions", filter: `user_id=eq.${userId}` },
        () => { loadData(); }, // Reload when new payment recorded
      )
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "node_allocations", filter: `user_id=eq.${userId}` },
        () => { loadData(); }, // Reload when new mining session starts
      )
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "users", filter: `id=eq.${userId}` },
        (payload) => {
          // Balance credited — update profile directly (no full reload needed)
          const updated = payload.new as any;
          if (updated?.balance_available !== undefined) {
            setProfile((prev: any) => prev ? { ...prev, balance_available: updated.balance_available, total_earned: updated.total_earned ?? prev.total_earned } : prev);
          }
        },
      )
      .subscribe();

    return () => { supabase.removeChannel(financeChannel); };
  }, [userId, loadData]);
  ...
  const activeNodes      = nodeAllocations.filter((n) => n.status === "active" && !n.mining_completed);
  const totalInvested    = nodeAllocations.reduce((s, n) => s + (n.amount_invested ?? 0), 0);
  const totalNodeEarned  = nodeAllocations.reduce((s, n) => s + (n.total_earned ?? 0), 0);
  const pendingWDs       = withdrawals.filter((w) => w.status === "queued" || w.status === "processing");
  ...
  const unclaimedMatured = nodeAllocations.filter(
    (n) => !n.mining_completed &&
           n.payment_model === "flexible" &&
           n.mining_ends_at &&
           new Date(n.mining_ends_at) <= new Date(),
  );
  const unclaimedCapital  = unclaimedMatured.reduce((s, n) => s + (n.amount_invested ?? 0), 0);
  const unclaimedEarnings = unclaimedMatured.reduce((s, n) => s + (n.total_earned ?? 0), 0);
  ...
  for (const pt of paymentTxs) {
    const meta = (() => { try { return pt.metadata ? JSON.parse(pt.metadata) : {}; } catch { return {}; } })();
    const isGpuPayment = pt.gateway === "gpu_mining" || meta.purchaseType?.includes("gpu");

    depositEntries.push({
      id:          `pt-${pt.id}`,
      type:        isGpuPayment ? (meta.purchaseType === "gpu_contract" ? "gpu_contract" : "gpu_mining") : "payment",
      label:       isGpuPayment
        ? `GPU Mining Deposit${meta.planName ? ` — ${meta.planName}` : ""}`
        : (meta.purchaseType === "license" ? "Operator License" : "Payment"),
      amount:      pt.amount,
      // FIX #6: gpu_mining gateway payments are treated as confirmed
      status:      isGpuPayment && pt.status !== "failed"
        ? "confirmed"
        : (pt.status === "confirmed" || pt.status === "confmrmed") ? "confirmed"
        : pt.status === "failed" ? "failed" : "pending",
      gateway:     pt.gateway,
  ...
  const ptReferences = new Set(paymentTxs.map((pt) => pt.gateway_reference).filter(Boolean));
  ...
  depositEntries.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  ...
  const canWithdraw       = !isFrozen && effectiveAvail >= 10 && !!profile?.payout_registered && kycOk;

  const TABS: Array<{ id: typeof tab; label: string; icon: React.ComponentType<{ size?: number; className?: string }> }> = [
  ...
  return (
    <div className="flex min-h-screen bg-slate-950 text-slate-200">
      ...
      {showWithdrawModal && userId && profile && (
        <WithdrawModal
          userId={userId} availableBalance={effectiveAvail} isFrozen={isFrozen} profile={profile}
          onClose={() => setShowWithdrawModal(false)}
          onSuccess={() => {
            setShowWithdrawModal(false);
            showToast("Withdrawal request submitted! Admin will process it shortly.");
            loadData();
            setTab("withdrawals");
          }} />
      )}
      ...
      {toast && (
        <div className={`fixed top-4 right-4 z-50 px-4 py-3 rounded-xl text-sm font-semibold shadow-xl max-w-sm flex items-center gap-2 ${toast.ok ? "bg-emerald-500 text-slate-950" : "bg-red-500 text-white"}`}>
          {toast.ok ? <CheckCircle size={14} /> : <XCircle size={14} />} {toast.text}
        </div>
      )}
        ...
        <div className="max-w-5xl mx-auto p-4 md:p-6 pb-24 md:pb-6 space-y-5">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div>
              ...
              <p className="text-slate-400 text-sm mt-0.5">Your complete financial picture</p>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowWithdrawModal(true)} disabled={!canWithdraw}
                ...
                {isFrozen               ? "Frozen"
                 : !profile?.payout_registered ? "Setup Payout First"
                 : !kycOk              ? "KYC Required"
                 : avail < 10          ? "Need $10 min"
          ...
          {!kycOk && (
            <div className="bg-amber-900/20 border border-amber-800/40 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-amber-300 text-sm">
                KYC verification required for withdrawals. Mining continues normally.{" "}
            ...
            <div className="bg-emerald-900/20 border border-emerald-700/40 rounded-xl px-4 py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-emerald-300 text-sm">
                  <strong>{unclaimedMatured.length} mining session{unclaimedMatured.length > 1 ? "s" : ""} completed</strong>{" "}
                  — ${unclaimedTotal.toFixed(2)} (capital + earnings) pending credit to your wallet.
                </p>
              </div>
              <button
                onClick={async () => {
          ...
          {kycOk && !profile?.payout_registered && (
            <div className="bg-amber-900/20 border border-amber-800/40 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-amber-300 text-sm">
                ...
                <button onClick={() => router.push("/dashboard/verification")} className="underline">
          ...
          {isFrozen && (
            <div className="bg-red-900/20 border border-red-800/40 rounded-xl px-4 py-3 flex items-center gap-3">
              ...
              <p className="text-red-300 text-sm">Your withdrawals are currently frozen. Contact support.</p>
            </div>
          )}
          {pendingWDs.length > 0 && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatBox label="Available Balance"  value={`$${effectiveAvail.toFixed(2)}`}         color="text-emerald-400" icon={DollarSign} sub="Ready to withdraw" />
            <StatBox label="Total GPU Invested" value={`$${totalInvested.toFixed(2)}`} color="text-violet-400"  icon={Cpu}        sub={`${activeNodes.length} active`} />
            <StatBox label="Total Deposited"    value={`$${totalDeposited.toFixed(2)}`}color="text-cyan-400"    icon={CreditCard}  sub={`${confirmedDeposits.length} confirmed`} />
            <StatBox label="Active Licenses"    value={activeLicenses.length}           color="text-amber-400"   icon={Shield}     sub={activeLicenses.length > 0 ? "Licensed" : "No license"} />
          </div>
            ...
            {TABS.map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => setTab(id)}
                ...
                {id === "withdrawals" && pendingWDs.length > 0 && (
                  <span className="text-[9px] font-black px-1.5 py-0.5 rounded-full bg-blue-500/20 border border-blue-500/30 text-blue-400">{pendingWDs.length}</span>
                )}
              </button>
            ))}
          </div>
          ...
          {tab === "overview" && (
            <div className="space-y-4">
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {[
                    ...
                    { label: "Pending Balance",   value: `$${pending.toFixed(2)}`,     color: "text-violet-400"  },
                    { label: "Locked Balance",    value: `$${locked.toFixed(2)}`,      color: "text-amber-400"   },
              ...
              <div className="rounded-2xl p-4 flex items-center justify-between gap-4"
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-slate-400 text-sm mt-0.5">
                    ...
                    {!kycOk ? "Complete KYC to enable withdrawals (mining is unaffected)."
                     : !profile?.payout_registered ? "Set up your payout account first."
                     : avail < 10 ? "You need at least $10 to withdraw."
              ...
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5">
                ...
                {profile?.payout_registered ? (
                  <div className="bg-slate-800/40 rounded-xl p-3 space-y-1">
                    <p className="text-white font-bold">{profile.payout_account_name}</p>
                    {profile.payout_bank_name && <p className="text-slate-400 text-sm">{profile.payout_bank_name}</p>}
                    <p className="text-slate-500 text-xs font-mono">{profile.payout_account_number}</p>
                    <p className="text-blue-400 text-xs capitalize">via {profile.payout_gateway}</p>
                    {kycOk
                      ? <p className="text-emerald-400 text-xs flex items-center gap-1"><CheckCircle size={10} /> KYC verified</p>
                      : <p className="text-amber-400 text-xs flex items-center gap-1"><AlertTriangle size={10} /> KYC verification pending</p>}
                  </div>
                ) : (
                  <div className="bg-amber-900/10 border border-amber-800/30 rounded-xl p-3">
                    ...
                    <p className="text-slate-500 text-xs mt-1">Register a payout account to enable withdrawals.</p>
              ...
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5">
                <p className="text-white font-bold text-sm mb-4 flex items-center gap-2">
                  <Activity size={14} className="text-blue-400" /> Performance Stats
                </p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    { label: "Tasks Completed", value: profile?.total_task_completed ?? 0,                                          color: "text-white"       },
                    { label: "Approved",        value: profile?.approved_count ?? 0,                                                color: "text-emerald-400" },
                    { label: "Rejected",        value: profile?.rejected_countb ?? 0,                                               color: "text-red-400"     },
                    { label: "Quality Score",   value: `${((profile?.qaulity_score ?? 0) * 100).toFixed(0)}%`,                     color: (profile?.qaulity_score ?? 0) >= 0.8 ? "text-emerald-400" : "text-amber-400" },
                    { label: "Streak",          value: `${profile?.streak_count ?? 0} days`,                                       color: "text-amber-400"   },
                    { label: "Referral Earned", value: `$${(profile?.referral_earnings ?? 0).toFixed(2)}`,                         color: "text-violet-400"  },
                    { label: "KYC Status",      value: kycOk ? "Verified ✓" : profile?.kyc_status ?? "Pending",                   color: kycOk ? "text-emerald-400" : "text-amber-400" },
                    { label: "Last Active",     value: profile?.last_active_at ? new Date(profile.last_active_at).toLocaleDateString() : "—", color: "text-slate-400" },
                  ].map(({ label, value, color }) => (
                    <div key={label} className="bg-slate-800/40 rounded-xl p-3">
                      <p className="text-slate-500 text-[10px] uppercase tracking-wide">{label}</p>
                      <p className={`font-black text-sm mt-0.5 ${color}`}>{value}</p>
          ...
          {tab === "deposits" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <p className="text-emerald-400 text-xs font-bold">${totalDeposited.toFixed(2)} confirmed</p>
              </div>

              {/* Summary */}
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-center">
                  <p className="text-slate-500 text-[10px] uppercase">GPU Mining</p>
                  <p className="text-emerald-400 font-black text-base">
                    {depositEntries.filter((d) => d.type === "gpu_mining" || d.type === "gpu_contract").length}
                  </p>
                </div>
                <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-center">
                ...
                <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-center">
                  ...
                  <p className="text-white font-black text-base">${totalDeposited.toFixed(0)}</p>
                </div>
              </div>
                    ...
                    return (
                      <div key={dep.id} className="border border-slate-800 rounded-xl overflow-hidden">
                        <button onClick={() => setExpanded(isOpen ? null : dep.id)}
                            ...
                            <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                              {dep.miningPeriod && dep.type !== "payment" && (
                                <span className="text-[10px] text-emerald-400/70">
                                  {PERIOD_LABELS[dep.miningPeriod] ?? dep.miningPeriod} session
                                </span>
                              )}
                              <span className="text-slate-500 text-[10px]">{new Date(dep.created_at).toLocaleString()}</span>
                            </div>
                          </div>
                          ...
                          {isOpen ? <ChevronUp size={13} className="text-slate-600 shrink-0" /> : <ChevronDown size={13} className="text-slate-600 shrink-0" />}
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 border-t border-slate-800 bg-slate-900/20">
                            <div className="bg-slate-800/40 rounded-xl p-3 space-y-1.5 text-xs mt-3">
                              {[
                                ...
                                ...(dep.gpuModel ? [["GPU", dep.gpuModel]] : []),
                                ...(dep.miningPeriod ? [["Session Duration", PERIOD_LABELS[dep.miningPeriod] ?? dep.miningPeriod]] : []),
                                ...(dep.reference   ? [["Reference ID", dep.reference.slice(0, 20) + "..."]] : []),
          ...
          {tab === "investments" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <p className="text-emerald-400 text-xs font-bold">${totalInvested.toFixed(2)} total committed</p>
              </div>
              ...
              {nodeAllocations.length === 0 ? (
                ...
                <div className="space-y-3">
                  {nodeAllocations.map((node) => {
                    ...
                    const maturesAt   = node.maturity_date   ? new Date(node.maturity_date)  : null;
                    ...
                    return (
                      <div key={node.id} className="border border-slate-800 rounded-2xl overflow-hidden">
                        <button onClick={() => setExpanded(isOpen ? null : `node-${node.id}`)}
                          className="w-full p-4 text-left hover:bg-slate-800/10 transition-colors">
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-8 h-8 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shrink-0">
                                {isContract ? <Cpu size={14} className="text-violet-400" /> : <Pickaxe size={14} className="text-emerald-400" />}
                              </div>
                              <div className="min-w-0">
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {!isComplete && <span className="text-[10px] text-emerald-400 font-bold flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />LIVE</span>}
                                </div>
                                <p className="text-slate-500 text-[10px] mt-0.5">
                                  {gpuModel} · {isContract ? "Contract" : PERIOD_LABELS[period] ?? period} session · {new Date(node.created_at).toLocaleDateString()}
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="px-4 pb-4 border-t border-slate-800 bg-slate-900/20">
                            <div className="bg-slate-800/40 rounded-xl p-3 space-y-1.5 text-xs mt-3">
                              {[
                                ...
                                ["Payment Model",    isContract ? `Contract — ${node.contract_label ?? ""}` : "Pay-As-You-Go"],
                                ["Session Duration", PERIOD_LABELS[period] ?? period],
                                ["Status",           isComplete ? "Complete ✓" : "Active ⛏️"],
                                ...(endsAt && !isContract ? [["Ends At", endsAt.toLocaleString()]] : []),
                                ...(maturesAt && isContract ? [["Matures", maturesAt.toLocaleString()]] : []),
                                ["Started",          new Date(node.created_at).toLocaleString()],
                                ...(node.final_profit != null ? [["Final Profit", `$${node.final_profit.toFixed(6)}`]] : []),
                              ].map(([l, v]) => (
                                <div key={l} className="flex justify-between gap-4">
                                  <span className="text-slate-500 shrink-0">{l}</span>
          ...
          {tab === "withdrawals" && (
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-center">
                  <p className="text-slate-500 text-[10px] uppercase">Total Withdrawn</p>
                  <p className="text-white font-black text-base">${totalWd.toFixed(2)}</p>
                </div>
                <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-center">
                  <p className="text-slate-500 text-[10px] uppercase">Pending</p>
                  <p className="text-blue-400 font-black text-base">{pendingWDs.length}</p>
                </div>
                <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-3 text-center">
                  <p className="text-slate-500 text-[10px] uppercase">Status</p>
                  <p className={`font-black text-base ${isFrozen ? "text-red-400" : "text-emerald-400"}`}>{isFrozen ? "Frozen" : "Open"}</p>
                </div>
              </div>
              ...
              <button onClick={() => setShowWithdrawModal(true)} disabled={!canWithdraw}
                ...
                {isFrozen               ? "Withdrawals Frozen"
                 : !kycOk              ? "KYC Verification Required (Mining still active)"
                 : !profile?.payout_registered ? "Setup Payout Account First"
                 : avail < 10          ? `Need $10 min (have $${effectiveAvail.toFixed(2)})`
                    ...
                    <div key={w.id} className="bg-slate-900/60 border border-slate-800 rounded-xl p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            {w.expected_date && w.status !== "paid" && (
                              <span className="text-[10px] text-slate-500">
                                Expected: {new Date(w.expected_date).toLocaleDateString()}
                              </span>
                            )}
                          </div>
                          ...
                          {w.paid_at && <p className="text-emerald-500 text-[10px] mt-0.5">✅ Paid: {new Date(w.paid_at).toLocaleString()}</p>}
                          {w.failure_reason && <p className="text-red-400 text-[10px] mt-1">❌ {w.failure_reason}</p>}
                        </div>
                        <p className="text-white font-black text-xl shrink-0">${w.amount.toFixed(2)}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              {licenses.length === 0 ? (
                ...
                licenses.map((lic) => {
                  ...
                  return (
                    <div key={lic.id} className={`rounded-2xl border p-5 ${lic.status === "active" && !expired ? "bg-emerald-900/10 border-emerald-800/40" : "bg-slate-900/60 border-slate-800"}`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-white font-black">{LICENSE_LABELS[lic.license_type] ?? lic.license_type}</p>
                          <p className="text-slate-400 text-xs">Purchased: {lic.purchased_at ? new Date(lic.purchased_at).toLocaleDateString() : "—"}</p>
                          {lic.expires_at && <p className="text-slate-500 text-xs">Expires: {new Date(lic.expires_at).toLocaleDateString()}</p>}
                        </div>
