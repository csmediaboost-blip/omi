...
export async function GET(req: NextRequest) {
  try {
    ...
    if (chargeStatus === "success" || chargeStatus === "completed") {
      ...
      // Get transaction details with proper field matching
      ...
      await supabase.from("node_allocations").insert({
        ...
        amount_invested: txData.amount, // Original USD amount
        amount_paid: txData.converted_amount || txData.amount, // Converted amount they paid
      ...
      // Credit user's account balance - use original USD amount
