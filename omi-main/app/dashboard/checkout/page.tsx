...
// RECEIPT FIXES:
//  [R-1] receiptData.amount now uses actualAmountPaid state (set at moment of confirmed payment)
//        instead of effectivePrice which reflects crypto discount regardless of actual method used.
//  [R-2] receiptData.payMethod now uses confirmedPayMethod state, set when payment is confirmed,
//        so KoraPay redirect no longer inherits stale UI state.
//  [R-3] countryName + countryCode restored from korapay_pending_checkout sessionStorage on
//        KoraPay success redirect so receipt country field is never blank.
//  [R-4] API access checkout (purchaseType missing, no miningPeriod) now detected — receipt
//        renders correct "API Developer Access" template instead of GPU mining template.
//  [R-5] receiptData.discounted only true when confirmedPayMethod is crypto_wallet.
//  [R-6] Split payment receipt uses totalUSD (original full price) as amount paid, not
//        effectivePrice which can be stale crypto-discounted value.
// PATCH (multi-slot KoraPay):
//  [P-1] getDailyBankTransferNGNTotal now queries converted_amount across ALL korapay slots
//        (gateway='korapay'), includes pending status, no manual NGN conversion needed.
//  [P-2] getTotalDailyCapacity: new fn — sums daily_limit_ngn from all active korapay_accounts.
//  [P-3] DAILY_NGN_LIMIT constant removed; capacity is now dynamic from DB.
//  [P-4] F1 useEffect uses Promise.all to fetch total + capacity together.
//  [P-5] handleBankTransferSubmit daily limit check uses dynamic capacity.
//  [P-6] ALL_ACCOUNTS_FULL error code from /api/korapay/checkout triggers setBankTransferBlocked.
...
// [P-3] DAILY_NGN_LIMIT constant removed — capacity is now fetched dynamically from korapay_accounts
...
// ─── F1: DAILY NGN TOTAL — ALL SLOTS COMBINED ────────────────────────────────
// [P-1] Queries converted_amount across ALL korapay slots (gateway='korapay').
// Includes 'pending' status and does not require manual NGN conversion since
// converted_amount is already stored in local currency by the API route.
async function getDailyBankTransferNGNTotal(): Promise<number> {
  ...
  try {
    const { data } = await supabase
      ...
      .eq("gateway", "korapay"); // all KoraPay slots, not just bank_transfer label

    if (!data || data.length === 0) return 0;

    return data.reduce(
      (sum: number, tx: { converted_amount: unknown }) =>
        sum + (Number(tx.converted_amount) || 0),
      0,
    );
  } catch {
    return 0;
...
// ─── DAILY TOTAL CAPACITY — ALL ACTIVE SLOTS ─────────────────────────────────
// [P-2] Sums daily_limit_ngn from all active rows in korapay_accounts.
// Falls back to 498_000 (single-slot default) if the table is empty or unreachable.
async function getTotalDailyCapacity(): Promise<number> {
  try {
    ...
    return (data as { daily_limit_ngn: number }[]).reduce(
      (s: number, row: { daily_limit_ngn: number }) =>
        s + Number(row.daily_limit_ngn),
      0,
    );
  } catch {
    return 498_000;
...
function Receipt({
  ...
  data: {
    txId: string;
    purchaseType: PurchaseType | "api_access";
    nodeName: string;
    amount: number;
    gpu: string;
    vram: string;
  ...
  const periodLabel =
    data.purchaseType === "api_access"
      ? "Lifetime"
      : PERIOD_LABELS[data.miningPeriod] ?? data.miningPeriod;
...
function CheckoutInner() {
  ...
  // [P-4] F1: Check daily bank transfer limit on mount using dynamic capacity
  useEffect(() => {
    ...
    if (s === "success") {
      ...
      if (saved) {
        ...
        try {
          ...
          console.error("[checkout] Failed to parse pending checkout data:", e);
        }
      }
    } else if (s === "declined") {
      sessionStorage.removeItem("korapay_pending_checkout");
  ...
  async function handleBankTransferSubmit() {
    ...
    // [P-5] Daily limit check uses dynamic capacity from all active KoraPay slots
    const [currentNGNTotal, totalCapacity] = await Promise.all([
      getDailyBankTransferNGNTotal(),
      getTotalDailyCapacity(),
    ]);
    if (currentNGNTotal >= totalCapacity) {
      setBankTransferBlocked(true);
      setKpError(
        "Bank transfer is unavailable — today's processing capacity across all accounts " +
        "has been reached. This resets at midnight. Please use Crypto or Card.",
      );
      setKpLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !data.checkoutUrl) {
        sessionStorage.removeItem("korapay_pending_checkout");
        // [P-6] If all KoraPay accounts are full, immediately block bank transfer in UI
        if (data.code === "ALL_ACCOUNTS_FULL") {
          setBankTransferBlocked(true);
        }
        setKpError(
          data.error || "Payment initiation failed. Please try again.",
  ...
  const receiptData = {
    ...
    nodeName,
    amount: actualAmountPaid ?? effectivePrice,
    gpu,
    vram,
    paymentModel,
    contractLabel,
    ...
    miningPeriod,
    payMethod:
      confirmedPayMethod === "bank_transfer"
        ? "Bank / Mobile Transfer"
        : confirmedPayMethod === "crypto_wallet"
          ? "Crypto Payment (USDT)"
          : "Credit / Debit Card",
    country: countryName,
    ...
    originalAmount: price,
    walletAddress:
      confirmedPayMethod === "crypto_wallet" ? twSenderAddress : undefined,
  };
                  ...
                  <div className="text-right">
                    ...
                    {payMethod === "crypto_wallet" && (
                      <p className="text-emerald-400 text-[10px]">
                        -{cryptoDiscount}% crypto discount
                      </p>
                    )}
                  </div>
                </div>
