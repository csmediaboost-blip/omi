...
// [P-3] DAILY_NGN_LIMIT constant removed — capacity fetched dynamically
...
// ─── [FIX-LOADING-2] fetchWithTimeout ────────────────────────────────────────
...
async function createMiningAllocation(params: {
  ...
  const {
    ...
    contractMinPct, contractMaxPct, lockInMonths, lockInLabel,
    lockInMultiplier, transactionRef, autoReinvest = false,
  } = params;
  ...
  const miningEndsAt = paymentModel === "flexible"
    ? new Date(now.getTime() + periodMs).toISOString()
    : null;
  const maturityDate = paymentModel === "contract" && contractMonths
  ...
  const allocationPayload: Record<string, any> = {
    user_id: userId, plan_id: planId, amount_invested: amount,
    status: "active", payment_model: paymentModel, instance_type: instanceType,
    total_earned: 0, total_withdrawn: 0, created_at: nowIso, updated_at: nowIso,
    auto_reinvest: autoReinvest,
    ...(paymentModel === "flexible" ? {
      mining_period: period, mining_ends_at: miningEndsAt,
      mining_completed: false, rate_factor_used: rateFactor,
      capital_returned: false, final_profit: 0,
    } : {}),
    ...(paymentModel === "contract" ? {
      contract_months: contractMonths, contract_label: contractLabel,
      contract_min_pct: contractMinPct, contract_max_pct: contractMaxPct,
      maturity_date: maturityDate, lock_in_months: lockInMonths,
      lock_in_label: lockInLabel, lock_in_multiplier: lockInMultiplier,
      mining_completed: false, rate_factor_used: rateFactor,
      mining_period: "contract", mining_ends_at: maturityDate,
    } : {}),
  };
  ...
  try {
    await supabase.from("payment_transactions").insert({
      user_id: userId, node_key: planId, amount, currency: "USD",
      gateway: "gpu_mining", gateway_reference: newAlloc.id,
      status: "confirmed", verified_by_admin: false,
      created_at: nowIso, confirmed_at: nowIso,
      metadata: JSON.stringify({
        purchaseType: paymentModel === "contract" ? "gpu_contract" : "gpu_mining",
        planName, gpuModel, miningPeriod: period,
        allocationId: newAlloc.id, transactionRef: transactionRef ?? null,
      }),
    });
  } catch {}
...
async function getDailyBankTransferNGNTotal(): Promise<number> {
  ...
  try {
    ...
    return data.reduce(
      (sum: number, tx: { converted_amount: unknown }) =>
        sum + (Number(tx.converted_amount) || 0), 0,
    );
  } catch { return 0; }
}
...
async function getTotalDailyCapacity(): Promise<number> {
  try {
    const { data } = await supabase
      ...
      .select("daily_limit_ngn")
      .eq("is_active", true);
    if (!data || data.length === 0) return 498_000;
    return (data as { daily_limit_ngn: number }[]).reduce(
      (s: number, row: { daily_limit_ngn: number }) => s + Number(row.daily_limit_ngn), 0,
    );
...
function SplitPaymentModal({
  state, loading, error, onInitiate, onCancel,
}: {
  state: SplitState; loading: boolean; error: string;
  onInitiate: (s: SplitState) => void; onCancel: () => void;
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg rounded-3xl overflow-hidden"
        style={{ background: "rgb(8,12,22)", border: "1px solid rgba(245,158,11,0.35)", boxShadow: "0 0 60px rgba(245,158,11,0.08)" }}>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <h3 className="text-white font-black text-lg leading-tight">
                {isFirst ? "Payment Split Required" : `Installment ${state.completed} of ${totalCount} Complete`}
              </h3>
              {!isFirst && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-[11px] leading-relaxed">
                ...
                <strong className="text-slate-300">No additional fees are applied.</strong>
              </p>
            </div>
          </div>
        </div>
            ...
            {state.installmentsNGN.map((amt, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between rounded-xl px-4 py-3"
                  style={{
                    background: isPaid ? "rgba(16,185,129,0.07)" : isCurrent ? "rgba(245,158,11,0.07)" : "rgba(15,23,42,0.6)",
                    border: isPaid ? "1px solid rgba(16,185,129,0.25)" : isCurrent ? "1px solid rgba(245,158,11,0.3)" : "1px solid rgba(255,255,255,0.05)",
                  }}>
                  <div className="flex items-center gap-3">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0 text-[10px] font-black"
                      style={{
                        background: isPaid ? "rgba(16,185,129,0.2)" : isCurrent ? "rgba(245,158,11,0.2)" : "rgba(100,116,139,0.15)",
                        color: isPaid ? "#10b981" : isCurrent ? "#f59e0b" : "#475569",
                      }}>
                      {isPaid ? <CheckCircle size={12} /> : i + 1}
                    </div>
                    <div>
                      <p className={`text-sm font-black ${isPaid ? "text-emerald-300" : isCurrent ? "text-amber-300" : "text-slate-500"}`}>
        ...
        {error && (
          <div className="mx-5 mt-3 rounded-xl p-3 flex items-start gap-2"
        ...
        <div className="px-5 py-5 space-y-3 mt-1">
          <button onClick={() => onInitiate(state)} disabled={loading}
...
function Receipt({
  ...
  data: {
    txId: string; purchaseType: PurchaseType | "api_access";
    nodeName: string; amount: number; gpu: string; vram: string;
    payMethod: string; country: string; date: string; paymentModel: string;
    contractLabel: string; contractMonths: number; licenseType: string;
    miningPeriod: string; discounted?: boolean; originalAmount?: number;
  ...
  const periodLabel = data.purchaseType === "api_access"
    ? "Lifetime" : PERIOD_LABELS[data.miningPeriod] ?? data.miningPeriod;
  const contractDurLabel = data.contractMonths === 6 ? "6 months"
    : data.contractMonths === 12 ? "12 months" : "2 years";
  ...
  const apiReceiptRows: [string, string][] = [
    ["Transaction ID", data.txId],
    ["Date & Time", data.date],
    ["Product", "API Developer Access"],
    ["Access Level", "Full — All Endpoints"],
    ["API Keys", "Up to 5 keys"],
    ["Daily Quota", "10,000 requests"],
    ["Amount Paid", `$${data.amount.toFixed(2)}`],
    ["Payment Method", data.payMethod],
    ["Country", data.country],
    ["Status", "Access Activated"],
  ];

  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="max-w-md w-full rounded-2xl overflow-hidden"
        style={{ background: "rgb(10,16,28)", border: "1px solid rgba(255,255,255,0.1)" }}
        onClick={(e) => e.stopPropagation()}>
        <div ref={ref}>
          <div className="p-6 text-center"
            style={{ background: "linear-gradient(135deg,rgba(16,185,129,0.15),rgba(6,12,24,0.9))" }}>
          ...
          <div className="px-6 py-5 space-y-3 text-sm">
            {data.purchaseType === "api_access"
              ? apiReceiptRows.map(([l, v]) => (
              ...
              : (data.purchaseType === "license"
                  ? [
                      ...
                      ["Amount Paid", `$${data.amount.toFixed(2)}${data.discounted ? " (Crypto discount applied)" : ""}`],
                      ["Payment Method", data.payMethod],
                      ["Country", data.country],
          ...
          <div className="px-6 py-4 text-center">
            <p className="text-slate-600 text-[11px] leading-relaxed">
              {data.purchaseType === "api_access"
                ? "Your API access is now active. Generate keys in Dashboard → API Access."
                : data.purchaseType === "license"
...
function OrderSummary({
  purchaseType, nodeName, gpu, vram, itype, paymentModel,
  contractLabel, contractMonths, price, licenseType,
  effectivePrice, cryptoDiscount, payMethod, miningPeriod,
}: {
  purchaseType: PurchaseType; nodeName: string; gpu: string; vram: string;
  itype: string; paymentModel: string; contractLabel: string; contractMonths: number;
  price: number; licenseType: string; effectivePrice: number; cryptoDiscount: number;
  payMethod: PayMethod; miningPeriod: string;
}) {
  const isContract = paymentModel === "contract";
  const contractDurLabel = contractMonths === 6 ? "6 months"
          ...
          <div className="rounded-2xl p-6 mb-4"
            ...
            <div className="space-y-4">
              {[
                ...
                ...(!isContract
                  ? [["Mining Duration", periodLabel], ["Earnings", "Live — visible in your portfolio after activation"]]
                  : [["Contract Term", contractDurLabel], ["Earnings", "Accumulate daily — visible in your portfolio"]]),
          ...
          {isContract ? (
            <div className="rounded-xl p-4 mb-4"
              ...
              <p className="text-amber-400/80 text-xs leading-relaxed">
                Capital of <strong className="text-amber-300">${price.toFixed(2)}</strong> is locked for{" "}
                <strong className="text-amber-300">{contractDurLabel}</strong>. Earnings accumulate every second
                and are visible in your portfolio. Capital released at maturity.
              </p>
            </div>
          ) : (
            <div className="rounded-xl p-4 mb-4"
              ...
              <p className="text-emerald-400/70 text-xs leading-relaxed">
                Mining runs for <strong className="text-emerald-300">{periodLabel}</strong>. When done,
                your capital + earnings are credited to your wallet immediately.
              </p>
            </div>
          )}
        </>
          ...
          <div className="rounded-2xl p-6 mb-4"
            ...
            <div className="flex items-start gap-4 mb-5 pb-5 border-b border-slate-700">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0"
                style={{ background: `${licConfig.color}18`, border: `1px solid ${licConfig.color}40` }}>
...
function CheckoutInner() {
  ...
  // Wipe stale sessions on fresh visit
  useEffect(() => {
  ...
  // ── [FIX-LOADING-1+2+3] Parallel init ──────────────────────────────────────
  useEffect(() => {
    let cancelled = false;

    // Auth fires immediately
    supabase.auth
      .getUser()
    ...
    // Config with 5s timeout — never hangs
    fetchWithTimeout("/api/checkout/payment-config", 5000)
      .then((r) => r.json())
      .then((data) => {
        if (cancelled) return;
        if (data.crypto_wallet_address) setCryptoWalletAddress(data.crypto_wallet_address);
        if (data.crypto_network) setCryptoNetwork(data.crypto_network);
        if (data.crypto_discount) setCryptoDiscount(Number(data.crypto_discount));
        if (data.crypto_qr_image_url) setCryptoQrImageUrl(data.crypto_qr_image_url);
        if (data.usd_to_ngn_rate && !isNaN(Number(data.usd_to_ngn_rate))) {
          CURRENCY_RATES["NG"] = { currency: "NGN", rate: Number(data.usd_to_ngn_rate) };
        }
        setConfigLoaded(true);
      })
      .catch(() => { if (!cancelled) setConfigLoaded(true); });

    // Daily limit — background, never blocks UI
    Promise.all([getDailyBankTransferNGNTotal(), getTotalDailyCapacity()])
      .then(([total, capacity]) => {
        if (!cancelled) {
          if (total >= capacity) setBankTransferBlocked(true);
      ...
      if (s === "success") {
        ...
        try {
          ...
          const updated: SplitState = {
            ...saved, completed: saved.completed + 1, references: [...saved.references, r],
          };
          if (updated.completed >= updated.installmentsNGN.length) {
            sessionStorage.removeItem("korapay_split_checkout");
            setTransactionId(r);
            setActualAmountPaid(updated.totalUSD);
            setConfirmedPayMethod("bank_transfer");
            if (updated.planData.countryCode) {
              setCountryCode(updated.planData.countryCode);
              setCountryName(updated.planData.countryName || "");
            }
            supabase.auth.getUser().then(
              async ({ data: { user } }: { data: { user: User | null } }) => {
                if (!user) return;
                try {
                  const id = await createMiningAllocation({
                    ...updated.planData, userId: user.id,
                    transactionRef: updated.references.join(","),
                  } as Parameters<typeof createMiningAllocation>[0]);
                  if (id) setAllocationId(id);
                } catch (e) { console.error("[checkout] Split allocation failed:", e); }
              });
            setStep("success");
          } else {
            sessionStorage.setItem("korapay_split_checkout", JSON.stringify(updated));
    ...
    if (s === "success") {
      ...
      if (saved) {
        ...
        try {
          ...
          const paidUSD = checkoutData.originalPrice ?? checkoutData.amount ?? price;
          setActualAmountPaid(paidUSD);
          setConfirmedPayMethod("bank_transfer");
          if (checkoutData.countryCode) {
            setCountryCode(checkoutData.countryCode);
            setCountryName(checkoutData.countryName || "");
          }
          supabase.auth.getUser().then(
            async ({ data: { user } }: { data: { user: User | null } }) => {
              if (!user) return;
              try {
                const id = await createMiningAllocation({
                  ...checkoutData, userId: user.id, transactionRef: r,
                } as Parameters<typeof createMiningAllocation>[0]);
                if (id) setAllocationId(id);
              } catch (e) { console.error("[checkout] Failed to create allocation after KoraPay:", e); }
            });
      ...
      if (saved) {
        try {
          ...
          if (savedData.countryCode) {
            ...
            setCountryName(savedData.countryName || "");
          }
          setActualAmountPaid(savedData.originalPrice ?? savedData.amount ?? price);
        } catch {}
      }
  ...
  useEffect(() => {
    ...
    setPayMethod(BANK_TRANSFER_COUNTRIES.has(countryCode) ? "bank_transfer" : "crypto_wallet");
  }, [countryCode]);

  // ── F2: Split installment ────────────────────────────────────────────────
  ...
  async function handleBankTransferSubmit() {
    ...
    const [currentNGNTotal, totalCapacity] = await Promise.all([
      getDailyBankTransferNGNTotal(), getTotalDailyCapacity(),
    ]);
    if (currentNGNTotal >= totalCapacity) {
      setBankTransferBlocked(true);
      setKpError("Bank transfer is unavailable — today's processing capacity across all accounts has been reached. This resets at midnight. Please use Crypto or Card.");
      setKpLoading(false);
      return;
    }
    ...
    if (convertedPrice > MAX_SINGLE_NGN_TXN) {
      ...
      const planData: Record<string, any> = {
        planId: nodeKey, planName: nodeName, amount: price, paymentModel,
        instanceType: itype, gpuModel: gpu, vram, miningPeriod, contractMonths,
        contractLabel, contractMinPct, contractMaxPct,
        lockInMonths: isContract ? contractMonths : lockInMonths,
        lockInLabel: isContract ? contractLabel : lockInLabel,
        lockInMultiplier, countryCode, countryName, autoReinvest,
        nodeKey, nodeName, itype, purchaseType, licenseType, originalPrice: price,
      };
      const newSplitState: SplitState = {
        totalNGN: convertedPrice, totalUSD: price,
        ngnRate: conversion?.rate ?? 1600, localCurrency,
        installmentsNGN, completed: 0, references: [], kpPhone: "", planData,
      };
    ...
    try {
      sessionStorage.setItem("korapay_pending_checkout", JSON.stringify({
        planId: nodeKey, planName: nodeName, amount: price, originalPrice: price,
        paymentModel, instanceType: itype, gpuModel: gpu, vram, miningPeriod,
      ...
      const res = await fetch("/api/korapay/checkout", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId, phone: "", nodeKey, nodeName,
          price: convertedPrice, originalPrice: price, currency: localCurrency,
          itype, gpu, vram, purchaseType, licenseType, paymentModel, miningPeriod,
          contractMonths, contractLabel, contractMinPct, contractMaxPct,
          lockInMonths: isContract ? contractMonths : lockInMonths,
      ...
      if (!res.ok || !data.checkoutUrl) {
        sessionStorage.removeItem("korapay_pending_checkout");
        if (data.code === "ALL_ACCOUNTS_FULL") setBankTransferBlocked(true);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (payMethod === "crypto_wallet") {
      ...
      try {
        ...
        const { error: insertErr } = await supabase.from("payment_transactions").insert({
          user_id: userId, node_key: nodeKey, amount: discountedPrice,
          currency: "USDT", gateway: "crypto", status: "pending",
          gateway_reference: txId, receiving_wallet: cryptoWalletAddress,
          crypto_wallet: twSenderAddress || null, crypto_network: cryptoNetwork,
          crypto_currency: "USDT", verified_by_admin: false,
          metadata: JSON.stringify({
        ...
        setActualAmountPaid(discountedPrice);
        setConfirmedPayMethod("crypto_wallet");
    ...
    try {
      const res = await fetch("/api/checkout", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId, nodeKey, amount: price, currency: "USD",
          itype, payMethod, countryCode, gateway: "card",
          purchaseType, licenseType, paymentModel, miningPeriod,
          contractMonths, contractLabel, contractMinPct, contractMaxPct,
          lockInMonths: isContract ? contractMonths : lockInMonths,
          lockInMultiplier, lockInLabel: isContract ? contractLabel : lockInLabel,
          autoReinvest,
      ...
      const id = await createMiningAllocation({
        ...
        lockInMonths: isContract ? contractMonths : lockInMonths,
        lockInLabel: isContract ? contractLabel : lockInLabel,
        lockInMultiplier, transactionRef: data.transactionId, autoReinvest,
      });
      if (id) setAllocationId(id);
      setTransactionId(data.transactionId || `TXN-${Date.now()}`);
      setActualAmountPaid(price);
      setConfirmedPayMethod("card");
      setStep("success");
      isSubmittingRef.current = false;
    } catch (err: unknown) {
  ...
  const receiptData = {
    txId: transactionId,
    purchaseType: (isApiAccess ? "api_access" : purchaseType) as PurchaseType | "api_access",
    nodeName, amount: actualAmountPaid ?? effectivePrice,
    gpu, vram, paymentModel, contractLabel, contractMonths, licenseType, miningPeriod,
    payMethod: confirmedPayMethod === "bank_transfer" ? "Bank / Mobile Transfer"
      : confirmedPayMethod === "crypto_wallet" ? "Crypto Payment (USDT)"
      : "Credit / Debit Card",
    country: countryName,
    date: new Date().toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" }),
    discounted: confirmedPayMethod === "crypto_wallet",
    originalAmount: price,
    walletAddress: confirmedPayMethod === "crypto_wallet" ? twSenderAddress : undefined,
  };
  ...
  return (
    <div className="min-h-screen py-8 px-4" style={{ background: "#0d1117" }}>
      ...
      {splitState !== null && step === "details" && (
        <SplitPaymentModal
          state={splitState} loading={splitLoading} error={splitError}
          onInitiate={initiateSplitInstallment}
          onCancel={() => {
            sessionStorage.removeItem("korapay_split_checkout");
            setSplitState(null); setSplitError(""); setPayMethod("crypto_wallet");
          }}
        />
      )}

      <div className="max-w-[960px] mx-auto mb-6">
        <button onClick={() => step === "details" ? setStep("country") : router.back()}
          className="flex items-center gap-1.5 text-slate-500 hover:text-slate-300 transition-colors text-sm">
          <ArrowLeft size={14} /> Back
        </button>
      </div>

      {/* ── [FIX-VERIFYING] Polling screen ── */}
      {step === "verifying" && (
      ...
      {step === "pending_crypto" && (
        <div className="max-w-[560px] mx-auto">
          <div className="rounded-3xl p-8"
            style={{ background: "rgba(22,28,36,0.95)", border: "1px solid rgba(139,92,246,0.3)" }}>
            ...
            <div className="rounded-2xl p-5 mb-5 space-y-4"
              ...
              <div className="flex justify-center">
                {cryptoQrImageUrl ? (
                  <img src={cryptoQrImageUrl} alt="Payment QR"
            ...
            <div className="rounded-xl p-3 mb-5"
              ...
              <p className="text-amber-400 text-xs leading-relaxed">
                <strong>Important:</strong> Send exactly <strong>{discountedPrice.toFixed(2)} USDT</strong> on
                the <strong>{cryptoNetwork}</strong> network only. Wrong network = lost funds.
              </p>
            </div>
            <button onClick={() => router.push("/dashboard")}
          ...
          <div className="rounded-2xl p-8"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[400px] overflow-y-auto">
              {filteredCountries.map((c) => (
                <button key={c.code} onClick={() => { setCountryCode(c.code); setCountryName(c.name); }}
            ...
            <button onClick={() => countryCode && setStep("details")} disabled={!countryCode}
              className={`w-full mt-6 py-3 rounded-lg font-bold transition-all flex items-center justify-center gap-2 ${
                countryCode ? "bg-emerald-600 hover:bg-emerald-500 text-white" : "bg-slate-700 text-slate-400 cursor-not-allowed"
              }`}>
        ...
        <div className="max-w-[960px] mx-auto">
          <div className="grid lg:grid-cols-[1fr_420px] gap-8">
            <OrderSummary
              ...
              contractMonths={contractMonths} price={price} licenseType={licenseType}
              effectivePrice={effectivePrice} cryptoDiscount={cryptoDiscount}
              payMethod={payMethod} miningPeriod={miningPeriod}
            />
            <div className="space-y-5">
              ...
              {bankTransferBlocked && dailyLimitChecked && (
                <div className="rounded-xl p-4 flex items-start gap-3"
                  ...
                  <div>
                    <p className="text-red-300 text-sm font-black">Bank Transfer Unavailable Today</p>
                    <p className="text-red-400/70 text-xs mt-0.5 leading-relaxed">
                      Our daily processing capacity for bank transfers has been reached for today. This resets
                      at midnight. Please use <strong className="text-red-300">Crypto</strong> or{" "}
              ...
              {purchaseType === "gpu_plan" && (
                <div className="rounded-xl p-4"
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <div onClick={() => setAutoReinvest((v) => !v)}
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-xs mt-0.5 leading-relaxed">
                        When your mining session completes, earnings automatically start a new {periodLabel} session.
              ...
              <form onSubmit={handleSubmit}>
                {payMethod === "bank_transfer" && !bankTransferBlocked && (
                  <div className="rounded-2xl p-6 space-y-4"
                    style={{ background: "rgba(22,28,36,0.95)", border: "1px solid rgba(59,130,246,0.25)" }}>
                    ...
                    <div className="p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                      <p className="text-blue-200 text-xs leading-relaxed">
                        You will be redirected to complete payment securely via bank transfer, card, or mobile money.
                      </p>
                    </div>
                    {localAmount && conversionInfo && (
                      <div className="p-3 rounded-lg"
                        ...
                        <p className="text-emerald-300 text-xs">
                          Approx. amount:{" "}
                          <strong className="text-emerald-200">{conversionInfo.currency} {localAmount.toLocaleString()}</strong>
                        </p>
                      </div>
                    )}
                    {kpError && (
                ...
                {payMethod === "crypto_wallet" && (
                  <div className="rounded-2xl p-6 space-y-5"
                    style={{ background: "rgba(22,28,36,0.95)", border: "1px solid rgba(139,92,246,0.25)" }}>
                    <div>
                      <p className="text-violet-300 font-black text-sm mb-1">Pay with USDT</p>
                      <p className="text-slate-500 text-xs">Scan the QR or copy the address, then send the exact amount.</p>
                    </div>
                    {!configLoaded ? (
                      ...
                      <div className="rounded-xl p-4 space-y-4"
                        ...
                        <div className="flex justify-center">
                          {cryptoQrImageUrl ? (
                            <img src={cryptoQrImageUrl} alt="Payment QR Code"
                        ...
                        <div>
                          <p className="text-slate-500 text-[10px] uppercase tracking-wider mb-1.5 font-bold">Wallet Address</p>
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="Paste your sending wallet address"
                    ...
                    <label className="flex items-start gap-2.5 cursor-pointer">
                      <div onClick={() => setTwConfirmed((v) => !v)}
                      ...
                      <p className="text-slate-300 text-xs leading-relaxed">
                        I understand I must send exactly <strong className="text-white">{discountedPrice.toFixed(2)} USDT</strong>{" "}
                        on the <strong className="text-white">{cryptoNetwork}</strong> network to the address above.
                      </p>
                    </label>
                    {cryptoError && (
                      <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-2">
              ...
              <div className="rounded-xl p-4"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className="text-white font-black text-xl">${effectivePrice.toFixed(2)}</span>
                    {payMethod === "crypto_wallet" && (
                      <p className="text-emerald-400 text-[10px]">-{cryptoDiscount}% crypto discount</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {PROCESSING_STEPS.map((ps, idx) => (
                <div key={ps.id} className={`flex items-center gap-3 text-sm ${idx <= processingStep ? "text-emerald-300" : "text-slate-600"}`}>
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                    idx < processingStep ? "bg-emerald-500 border-emerald-500"
                    : idx === processingStep ? "border-emerald-500 animate-pulse"
                    : "border-slate-600"
                  }`}>
        ...
        <div className="max-w-[520px] mx-auto">
          <div className="rounded-3xl p-8"
            ...
            <h2 className="text-white font-black text-2xl text-center mb-2">
              {isApiAccess ? "API Access Activated!" : purchaseType === "license" ? "License Activated!" : "Mining Session Started!"}
            </h2>
            <p className="text-slate-400 text-sm text-center mb-6">
              {isApiAccess
                ? "Your API developer access is now active. Generate keys in Dashboard → API Access."
                : purchaseType === "license"
            ...
            <div className="rounded-xl p-4 mb-6 space-y-3 text-sm"
              style={{ background: "rgba(0,0,0,0.35)", border: "1px solid rgba(255,255,255,0.06)" }}>
              {[
                ["Transaction ID", transactionId],
                ["Amount Paid", `$${(actualAmountPaid ?? effectivePrice).toFixed(2)}`],
                ...(isApiAccess
                  ? [["Access", "Full API — All Endpoints"]]
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => router.push(
                  isApiAccess ? "/dashboard/api-access"
                  : purchaseType === "license" ? "/dashboard/tasks"
                  : "/dashboard/gpu-plans"
                )}
                className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-lg transition-all">
                {isApiAccess ? "Go to API Access →" : purchaseType === "license" ? "Go to Tasks" : "View Portfolio →"}
              </button>
            </div>
          </div>
        </div>
      ...
      {step === "failed" && (
        <div className="max-w-[520px] mx-auto">
          <div className="rounded-3xl p-8"
            ...
            <div className="flex gap-3">
              <button onClick={() => { setErrorMsg(""); isSubmittingRef.current = false; setStep("details"); }}
