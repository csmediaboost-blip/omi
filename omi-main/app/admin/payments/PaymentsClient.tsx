...
//  FIX-6  Dispatch "admin-record-viewed" so the notification bell clears
//         the "New Payment Received" badge once admin opens the detail modal
// ─────────────────────────────────────────────────────────────────────────────
...
export default function PaymentsClient({
  ...
  // ── Notify the admin notification bell that this payment was viewed ──────
  // This clears the corresponding "New Payment Received" notification (if
  // any) so the unread badge count goes down once the admin opens the
  // detail modal for that specific payment.
  function markPaymentViewed(paymentId: number) {
    window.dispatchEvent(
      new CustomEvent("admin-record-viewed", {
        detail: { category: "payment", recordId: String(paymentId) },
      }),
    );
  }
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={() => {
                            ...
                            setTxHashInput("");
                            // Clear any "New Payment Received" notification
                            // in the admin bell for this payment now that
                            // it's been viewed.
                            markPaymentViewed(p.id);
                          }}
